...
pub struct WeakLearner {
    ...
    /// threshold, o dado será classificado como 1, caso contrário, será classificado como -1.
    pub threshold: i64,
    /// Polaridade da decisão. Se for 1, o dado será classificado como 1 se for maior que o threshold,
    /// caso contrário, será classificado como -1. Se for -1, o dado será classificado como 1 se for
    /// menor que o threshold, caso contrário, será classificado como -1.
    pub polarity: i64,
...
impl WeakLearner {
    ...
    /// for menor que o threshold e a polaridade for -1. Caso contrário, a classificação é definida como -1.
    /// 
            ...
            for threshold in min_categorial_feature_value..=max_categorical_feature_value {
                for polarity in [-1, 1].iter() {
                    ...
                    for sample in &weighted_data.samples {
                        let predicted_label = if (sample.features[feature_index] - threshold) * (*polarity) >= 0 { 1 } else { -1 };
    ...
    pub fn predict(&self, features: Vec<i64>) -> i64 {
        ...
        if (features[self.feature_index] - self.threshold) * (self.polarity) >= 0 { 1 } else { -1 }
    }
}
